...
mapMeasureHint: $('mapMeasureHint'),
  ...
  function updatePresetRouteUI() {
    ...
    if (els.mapRouteSelectValue) {
      els.mapRouteSelectValue.textContent = `Выбран: ${activeRoute.label}`;
    }
    ...
    if (els.mapRouteStartValue) {
      els.mapRouteStartValue.textContent = showRouteStart
        ? `Выбрано начало: ${getRouteStartLabel(mapPresetRouteStart)}`
        : '';
    }

    if (els.mapPresetRouteLabel) {
      els.mapPresetRouteLabel.hidden = !mapPresetRouteVisible;
      els.mapPresetRouteLabel.textContent = showRouteStart
        ? `${activeRoute.label} · старт: ${getRouteStartLabel(mapPresetRouteStart)}`
        : activeRoute.label;
    }
  }
